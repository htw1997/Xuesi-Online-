...
function fun(path) {
    ...
    layui.use('layer', function () {
        ...
        var loadsrc = "<video  width='100%' height='100%'  controls='controls' autoplay='autoplay'>" +
            "<source src=" + path + "'../html' type='video/mp4'></source>" +
            "</video>"
...
layui.use(['table', 'upload', 'layer', 'laydate'], function () {
    ...
    var tableIns = table.render({
        ...
        url: 'http://localhost:8080/xuesi/speakers/getAll',
        //表格的名称
        title: '讲师表',
        //开启分页
        page: true,
    ...
    table.on('toolbar(videotest)', function (obj) {
        ...
        switch (obj.event) {
            ...
            case 'delete':
                if (data.length === 0) {
                    ...
                    layer.confirm("数据可贵! 是否确认删除?", function (index) {
                        ...
                        $.ajax({
                            type: "post",
                            url: "http://localhost:8080/xuesi/speakers/del",
                            dataType: 'json',
                            data: {
                                //key     value
                                "ids": ids
            ...
            case 'up':
                ...
                break;
            case 'down':
                window.location.href="http://localhost:8080/xuesi/videos/down?filename=moban.xlsx"
                break;
        }
        ;
    });
    ...
    table.on('edit(videotest)', function (obj) {
        ...
        $.ajax({
            type: "post",
            url: "http://localhost:8080/xuesi/speakers/update",
            dataType: "json",
            data: {
                "vid": data.id,
                "field": field,
    ...
    $("#re").on("click", function () {
        console.log($("#speakerId").val())
        console.log($("#speakerDesc").val())
        //需要获取到输入框的值,然后请求后台,获取数据以后重新加载我们的数据表格
        table.reload('videodemo', {
            page: {
                curr: 1 //重新从第 1 页开始
            },
            url: "http://localhost:8080/xuesi/speakers/selectByLike",
            where: {//设置异步请求的参数
                "vid": $("#speakerId").val(),
    ...
    table.on('tool(videotest)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'edit') {
            ...
            layer.open({
                ...
                title: '编辑视频信息',
                area: ['60%', '60%'],
                content: '/xuesi/speakers/getEdit?vid='+data.id
            })
