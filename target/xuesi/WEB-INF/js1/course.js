...
layui.use(['table', 'upload', 'layer', 'laydate'], function () {
    ...
    var tableIns = table.render({
        ...
        url: 'http://localhost:8080/xuesi/course/getAll',
        //表格的名称
        title: '课程表',
        //开启分页
        page: true,
            ...
            case 'delete':
                if (data.length === 0) {
                    ...
                    layer.confirm("数据可贵! 是否确认删除?", function (index) {
                        ...
                        $.ajax({
                            type: "post",
                            url: "http://localhost:8080/xuesi/course/del",
                            dataType: 'json',
                            data: {
                                //key     value
                                "ids": ids
    ...
    table.on('edit(videotest)', function (obj) {
        ...
        $.ajax({
            type: "post",
            url: "http://localhost:8080/xuesi/course/update",
            dataType: "json",
            data: {
                "vid": data.id,
                "field": field,
    ...
    $("#re").on("click", function () {
        ...
        table.reload('videodemo', {
            ...
            url: "http://localhost:8080/xuesi/course/selectByLike",
            where: {//设置异步请求的参数
                "vid": $("#courseId").val(),
    ...
    $("#sel").on("click", function () {
        ...
        table.reload('videodemo', {
            ...
            url: "http://localhost:8080/xuesi//getAll",
            where: {//设置异步请求的参数
                "stu": $("#selStu").val(),
    ...
    table.on('tool(videotest)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'edit') {
            ...
            layer.open({
                ...
                title: '编辑视频信息',
                area: ['60%', '60%'],
                content: '../course/getEdit?vid=' + data.id
            })
